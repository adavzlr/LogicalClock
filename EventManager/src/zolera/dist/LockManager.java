...
public class LockManager {
	...
	public static synchronized void lock(String res) {
		...
		CommunicationsManager.prepareMsgForAll(msg);
	...
	public static synchronized void processReleaseMessage(MutexMessage msg) {
		...
		// If we become the new lock owner, wake up the process.
		if (haveLock(resource)) {
	...
	private static void printQueue(Queue<LockRequest> q) {
		for(LockRequest req : q) {
			System.out.print("["+req.requester+"/"+req.clock+"/"+req.acknowledges+"] ");
		}
		System.out.println();
	}
